...
quantile(dat$Murder)
```
*Answer: The mean is 7.788.  The median is 7.25.  Q1 is 4.075 and Q3 is 11.250. Mean and median are both measures of center; however, the mean is the average value of a data set, which is calculated by adding all of the values and dividing by the number of total data points, and the mean is calculated by finding the middle value when the data set is arranged in order.  A quartile is essentially the same as a median, but with 25% and 75% of the data, rather than 50%. R gives you quartiles in order to help visualize the data and see if it is distributed symmetrically.*
...
*Answer: The par command sets parameters for graphs, changes how they are displayed, and allows us to tell R to compare additional variables within the data set. The parameters of (1,3) tell it to display them stacked on top of each other, rather than side by side or otherwise.*

What can you learn from plotting the histograms together?

*Answer: We can learn that assault arrest rates per 100,000 people are far higher than the arrest rates for murder and rape by comparing the x axis of all three histograms.  Additionally, while the frequency of arrest rates for rape and murder skew relatively right, the frequency of assault arrest rates are more uniform or have closer to a normal distribution.Comparing and plotting them together can generally help us compare distributions overall*
...
#  geom_map(map=map_data("state")) + 
#  expand_limits(x=map_data("state")$long, y=map_data("state")$lat)
#- This line tells R to use ggplot and to draw information from dat, and then to draw a map with states, the new variable that we created, specifically using numbers from the Murder column. It additionally tells R to make the x and y axes and limits coordinate with longitude and latitude in a US map.
```
